...
```bash
python -m src.localespatial.mcp_server.server        # MCP server (streamable-http on :8000)
python -m src.localespatial.viz.payload              # regenerate the viz sample from the mock
open src/localespatial/viz/app/index.html            # interactive tissue map
```
...
The linchpin is the shared data contract in `src/localespatial/schema.py` (Pydantic v2)
...
### Lane A: data + analysis engine (`src/localespatial/data/`, `src/localespatial/engine/`)
...
```
src/localespatial/schema.py        shared data contract (the coordination artifact)
src/localespatial/data/            build the canonical AnnData (Lane A)
src/localespatial/engine/          spatial analysis engine (Lane A)
src/localespatial/mcp_server/      MCP tools (Lane B)
src/localespatial/viz/             tissue-map payload + widget (Lane C)
